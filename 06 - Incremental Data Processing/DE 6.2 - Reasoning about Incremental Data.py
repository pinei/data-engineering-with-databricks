...
# MAGIC FROM streaming_tmp_vw
# MAGIC ORDER BY time
# MAGIC -- Sorting is not supported on streaming DataFrames/Datasets, unless it is on aggregated DataFrame/Dataset in Complete output mode
...
# MAGIC
# MAGIC **NOTE:** **`Trigger.AvailableNow`**</a> is a new trigger type that is available in DBR 10.1 for Scala only and available in DBR 10.2 and above for Python and Scala. In Databricks Runtime 11.3 LTS and above, the **`Trigger.Once`** setting is deprecated.
